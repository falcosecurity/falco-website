---
exclude_search: true
title: 条件と出力でサポートされるフィールド
weight: 3
---

# はじめに

Falcoがサポートするフィールドは次のとおりです。これらのフィールドは、Falcoルールの`condition` keyと`output` keyで使用できます。ルールの `output`keyに含まれるフィールドは、`json_output`が`true`に設定されている場合、アラートの`output_fields`オブジェクトにも含まれます。
...
ファイル記述子のフィールド。 ネットワークとファイル/ディレクトリフィールドが含まれます。
...
## `proc` & `thread` フィールドクラス

プロセスまたはスレッドを実行または生成するためのフィールド
...
システムコールイベントのフィールド。システムコール名を指定するには、`evt.type`を使用します。Falcoの `syscall`ルールごとに少なくとも1つの `evt` フィールドが必要です。
...
syslogに送信されるメッセージに関連するフィールド。Syslogメッセージに基づいてルールを作成できるようにします。
...
コンテナに関連するフィールド。 `container.name`、`container.image`などに基づいてフィルタリングできます。
```
...
Kubernetesメタデータでフィルタリングするフィールド。ルールを特定のネームスペース (`k8s.ns.name`)またはリソースのラベルに適用できるようにします。
...
## `jevt` フィールドクラス: jsonイベントにアクセスする一般的な方法
```
...
## `ka` フィールドクラス: K8s Auditログイベントへのアクセス
```
