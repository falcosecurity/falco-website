...
요소 | 설명
:-------|:-----------
[규칙](#rules) | 알림이 생성되어야 하는 *조건*이다. 규칙은 알림과 함께 전송되는 설명형의 *출력 문자열*과 함께 제공된다.
[매크로](#macros) | 규칙 및 다른 매크로 안에서 재사용 될 수 있는 규칙 조건 스니펫. 매크로는 공통 패턴의 이름을 지정하고 규칙의 중복을 제거하는 방법을 제공한다.
[리스트](#lists) | 규칙, 매크로 또는 기타 리스트에 포함될 수 있는 항목 모음이다. 규칙 및 매크로와 달리 목록은 필터링 식으로 구문을 분석할수 없다.
...
`warn_evttypes` | no | `false` 로 설정하면, 팔코는 이벤트 유형이 없는 규칙과 관련된 경고를 표시하지 않는다(자세한 내용은 [아래](#rule-condition-best-practices)). | `true`
