...
The Falco package will look into your system for the `dialog` binary, if the binary is there, the package will prompt a simple configuration dialog. If it is not present, the installation script will try to use the `modern_ebpf` driver wherever it is supported, falling back at other drivers where needed features are not available. The same autoselection can be triggered by selecting `Automatic selection` in the `dialog` prompt.
