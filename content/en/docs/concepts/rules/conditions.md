...
`regex` | Checks whether a string field matches a regular expression written in POSIX (egrep) syntax. Note that `regex` can be considerably slower than simpler string operations. The `regex` operator performs full match only, not partial match (i.e. anchored to both the beginning and the end). Example: `fd.name regex '[a-z]*/proc/[0-9]+/cmdline'`.
