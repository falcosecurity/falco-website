...
Use a space before and after the modifier. Put the right-hand values in parentheses. You can also reference a [Falco list](/docs/concepts/rules/basic-elements/#lists) inside those parentheses, for example `fd.name contains anyof (suspicious_filename_bytes)`. Modifiers work with `=`, `==`, `!=`, `contains`, `icontains`, `bcontains`, `startswith`, `bstartswith`, `endswith`, `glob`, `iglob`, and `regex`, subject to the field's supported operators. They do not apply to `in`, `intersects`, `pmatch`, `exists`, or numeric ordering operators such as `>`.
